...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Kunci biar data siswa & baris tabel tidak patah terpotong halaman */
        .student-group { page-break-inside: avoid; display: block; margin-bottom: 15px; }
...
<div class="page">
    ...
    <table class="kop-table">
        <tr>
            ...
            <td class="kop-text">
                ...
                <div class="doc-title">Riwayat Absensi Siswa</div>
            </td>
            <td class="kop-badge">
    ...
    {{-- ── 2. INFO PERIODE & ORANG TUA ── --}}
    <table class="info-table">
        <tr>
            <td style="width:50%; padding-right:6px;">
                <div class="info-card">
                    <div class="lbl">Orang Tua / Wali</div>
                    <div class="val">{{ $parentName }}</div>
                </div>
            </td>
            <td style="width:50%;">
                <div class="info-card">
        ...
        <tr>
            <td>
                <div class="summary-box box-total">
                    <div class="s-count">{{ $totalRecord }}</div>
   ...
   @foreach($grouped as $studentName => $studentAbsences)
        ...
        <div class="student-header">
            Siswa: {{ $studentName }} &nbsp;|&nbsp;
            Kelas: {{ $studentAbsences->first()->student->class?->name ?? '-' }} &nbsp;|&nbsp;
        ...
        <table class="main-table">
            ...
            <tbody>
                @forelse($studentAbsences as $i => $absence)
                @php
                ...
                <tr>
                    ...
                    <td>{{ $absence->attendance_time->translatedFormat('l') }}</td>
                    <td>{{ $absence->attendance_time->format('H:i') }}</td>
                    <td>{{ $absence->checkout_time ? $absence->checkout_time->format('H:i') : '-' }}</td>
                    <td>
                        <span class="pill {{ $pillClass }}">
                            {{ $status }}{{ $absence->checkout_time ? ' (Selesai)' : '' }}
                        </span>
                    </td>
                </tr>
                @empty
