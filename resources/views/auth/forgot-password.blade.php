    ...
    <div class="w-full max-w-[420px]">
        ...
        <div class="flex justify-center mb-6">
            <div class="inline-flex items-center gap-2 border border-[var(--line)] bg-white px-3 py-1.5 rounded-md">
                ...
                <span class="font-mono text-[10px] tracking-[0.16em] uppercase text-[var(--muted)]">Pemulihan Akun &middot; {{ $schoolName }}</span>
            </div>
        </div>
        ...
        <div class="bg-white border border-[var(--line)] rounded-xl shadow-[0_1px_2px_rgba(20,23,28,0.04)] p-7 sm:p-9">
            ...
            <div class="mb-6 text-center">
                <h1 class="font-display text-2xl font-bold text-[var(--ink)] tracking-tight">Lupa Password?</h1>
                <p class="text-sm text-[var(--muted)] mt-1.5">Masukkan email terdaftar untuk menerima link pemulihan</p>
            </div>
            ...
            <form action="{{ route('password.email') }}" method="POST" class="space-y-5" id="forgotForm">
                @csrf
                ...
                <p class="text-center text-sm text-[var(--muted)]">
                    Ingat password Anda?
                    <a href="{{ route('login') }}" class="font-bold text-[var(--ink)] hover:text-[var(--accent)]">Kembali login</a>
                </p>
            </form>
        </div>
